...
#define find_stack_start(addr)                                 \
    ...
    ((addr > (void *)0xc0000000) ? NULL :                      \
         ...
         (addr > (void *)0xbf800000) ? (void *)0xc0000000 :    \
                                                               \
                                     /* Other thread stacks */ \
             ((void *)(((uint)addr & (~PTHREAD_STACK_SIZE)) +  \
                       PTHREAD_STACK_SIZE))                    \
                                                               \
    )
...
int _libsafe_verify_ra_fp(int maxcount, caddr_t *ra_array, caddr_t *fp_array) {
    ...
    while ((fp <= stack_start)) {
        ...
        if (ra_array[count] != *(caddr_t*)(fp+sizeof(void*)) ||
            ...
            count++ > maxcount)
        */
        if (*ra_p++ != *(caddr_t *)(fp + sizeof(void *)) || *fp_p++ != fp ||
            count++ > maxcount) {
            /*
             * Mismatch found!
             */
...
void create_dump_stack_filename(char *filename, int size) {
    ...
    for (p = LIBSAFE_DUMP_STACK_FILE, p2 = filename; *p && count < size - 1;
         p++) {
        *p2++ = *p;
        count++;
    }
...
void _libsafe_dump_stack(char *file, int linenum) {
    ...
    fprintf(fp, "Stack dump:  sp=%p  fp=%p  stack_start=%p\n", sp,
            __builtin_frame_address(0), current_stack_start);
     ...
     * to avoid segfaults.
     */
    for (; sp <= (char *)current_stack_start + 0x40 && sp < (char *)0xbffffffb;
         sp += 8) {
        fprintf(fp, "%p:  %02x %02x %02x %02x %02x %02x %02x %02x\n", sp,
                *sp & 0xff, *(sp + 1) & 0xff, *(sp + 2) & 0xff,
                *(sp + 3) & 0xff, *(sp + 4) & 0xff, *(sp + 5) & 0xff,
                *(sp + 6) & 0xff, *(sp + 7) & 0xff);
    }
...
static void sendmail(char *recipient, char *message) {
    ...
    if (send_command(s, -1,
                     ...
                     "%s\r\n",
                     recipient, ctime_nonewline(&t), hostname, ctime(&t),
                     message))
        return;
    if (send_command(s, 2, "\r\n.\r\n")) return;
...
static int set_user_infos(idmef_user_t *user) {
    ...
    uent = getpwuid(uid);
    if (uent) idmef_string_set(&userid->name, uent->pw_name);
    ...
    userid->number = gid;
    gent = getgrgid(gid);
    if (gent) idmef_string_set(&userid->name, gent->gr_name);
...
static void prelude_alert(char *filename) {
    ...
    if (!classification) goto err;

    idmef_string_set_constant(&classification->name, "Stack Overflow Attempt");
 ...
 */
static int find_caller_addr(struct maps_st *maps, int count,
                            caddr_t caller_addr) {
    int i;
...
void _libsafe_warn(char *format, ...) {
...

    {
        ...
        for (fp = __builtin_frame_address(0); *fp; fp = nextfp) {
            ...
            nextfp = *(void **)fp;
            if (check_nextfp(fp, nextfp)) break;
        }
    }
#ifndef DEBUG_TURN_OFF_SYSLOG
...
#ifdef NOTIFY_WITH_EMAIL
    {
        ...
        snprintf(errmsg, sizeof(errmsg),
                 ...
                 "Terminating %s.\r\n"
                 "    uid=%d  euid=%d  pid=%d\r\n",
                 VERSION, exename, getuid(), geteuid(), getpid());
        {
            /*
