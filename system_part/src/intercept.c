...
typedef int (*vsnprintf_t)(char *str, size_t size, const char *format,
                           va_list ap);
...
typedef int (*_IO_vfscanf_t)(FILE *s, const char *format, va_list argptr,
                             int *errp);
...
char *strcpy(char *dest, const char *src) {
    ...
    if (!real_memcpy) real_memcpy = (memcpy_t)getLibraryFunction("memcpy");
    if (!real_strcpy) real_strcpy = (strcpy_t)getLibraryFunction("strcpy");

    if (_libsafe_exclude) return real_strcpy(dest, src);
...
char *strncpy(char *dest, const char *src, size_t n) {
    ...
    if (!real_strncpy) real_strncpy = (strncpy_t)getLibraryFunction("strncpy");

    if (_libsafe_exclude) return real_strncpy(dest, src, n);
...
char *stpcpy(char *dest, const char *src) {
    ...
    if (!real_memcpy) real_memcpy = (memcpy_t)getLibraryFunction("memcpy");
    if (!real_stpcpy) real_stpcpy = (stpcpy_t)getLibraryFunction("stpcpy");

    if (_libsafe_exclude) return real_stpcpy(dest, src);
...
wchar_t *wcscpy(wchar_t *dest, const wchar_t *src) {
    ...
    if (!real_wcscpy) real_wcscpy = (wcscpy_t)getLibraryFunction("wcscpy");

    if (_libsafe_exclude) return real_wcscpy(dest, src);
...
wchar_t *wcpcpy(wchar_t *dest, const wchar_t *src) {
    ...
    if (!real_wcpcpy) real_wcpcpy = (wcpcpy_t)getLibraryFunction("wcpcpy");

    if (_libsafe_exclude) return real_wcpcpy(dest, src);
...
void *memcpy(void *dest, const void *src, size_t n) {
    ...
    if (_libsafe_exclude) return real_memcpy(dest, src, n);
    ...
    if (n > max_size) _libsafe_die("Overflow caused by memcpy()");
    return real_memcpy(dest, src, n);
}
...
char *strcat(char *dest, const char *src) {
    ...
    if (!real_memcpy) real_memcpy = (memcpy_t)getLibraryFunction("memcpy");
    if (!real_strcat) real_strcat = (strcat_t)getLibraryFunction("strcat");

    if (_libsafe_exclude) return real_strcat(dest, src);
...
char *strncat(char *dest, const char *src, size_t n) {
    ...
    if (!real_strncat) real_strncat = (strncat_t)getLibraryFunction("strncat");

    if (_libsafe_exclude) return real_strncat(dest, src, n);
...
wchar_t *wcscat(wchar_t *dest, const wchar_t *src) {
    ...
    if (!real_memcpy) real_memcpy = (memcpy_t)getLibraryFunction("memcpy");
    if (!real_wcscat) real_wcscat = (wcscat_t)getLibraryFunction("wcscat");

    if (_libsafe_exclude) return real_wcscat(dest, src);
...
int vfprintf(FILE *fp, const char *format, va_list ap) {
    ...
    for (p = (char *)format; *p; p++) {
        if (*p == '%') {
             ...
             */
            if (*p == '*') {
                for (p++, in_mth = 0; *p && isdigit((int)*p); p++) in_mth = 1;
                if (*p == (char)NULL) break;
                if (in_mth) {
                    if (*p == '$') {
                        p++;
            ...
            if (is_printf_convspec[(int)*p]) {
                ...
                if (pnum) {
                    addr =
                        *((caddr_t *)(ap + (atoi(pnum) - 1) * sizeof(char *)));
                } else {
...
int vsprintf(char *str, const char *format, va_list ap) {
    ...
    if (_libsafe_exclude) return real_vsprintf(str, format, ap);
...
int vsnprintf(char *str, size_t size, const char *format, va_list ap) {
    ...
    if (_libsafe_exclude) return real_vsnprintf(str, size, format, ap);
...
char *getwd(char *buf) {
    ...
    if (!real_getwd) real_getwd = (getwd_t)getLibraryFunction("getwd");

    if (_libsafe_exclude) return real_getwd(buf);
...
char *gets(char *s) {
    ...
    if (!real_gets) real_gets = (gets_t)getLibraryFunction("gets");

    if (_libsafe_exclude) return real_gets(s);
...
char *realpath(char *path, char resolved_path[]) {
    ...
    if (_libsafe_exclude) return real_realpath(path, resolved_path);
...
int _IO_vfscanf(FILE *s, const char *format, va_list argptr, int *errp) {
    ...
    if (_libsafe_exclude) return real_IO_vfscanf(s, format, argptr, errp);
    ...
    if (save_count >= 0 &&
        _libsafe_verify_ra_fp(save_count, ra_array, fp_array) == -1) {
    ...
    if ((fp = fopen("/etc/libsafe.exclude", "r")) != NULL) {
        while (fgets(omitfile, sizeof(omitfile), fp)) {
            ...
            if (!strncmp(omitfile, "LIBSAFE_PROTECT_ROOT", sizeof(omitfile))) {
                if (geteuid() >= 100) _libsafe_exclude = 1;
            }
        }
...
static void _intercept_fini(void) { LOG(4, "end of _intercept_fini()\n"); }
